...
// 2. Corrige el formato de las funciones para que sea C++ (void) y no JavaScript
Blockly.JavaScript.forBlock['procedures_defnoreturn'] = function(block) {
    const funcName = Blockly.JavaScript.nameDB_.getName(block.getFieldValue('NAME'), 'PROCEDURE');
...
// 1. Dile a Blockly que ignore estos bloques en la generación general
Blockly.JavaScript.forBlock['robot_setup'] = function(block) {
    return ''; // No genera nada aquí porque lo extraes manualmente luego
};

Blockly.JavaScript.forBlock['robot_loop'] = function(block) {
    return ''; // No genera nada aquí porque lo extraes manualmente luego
};

Blockly.JavaScript.forBlock['text'] = function(block) {
    const text = block.getFieldValue('TEXT');
    // Si es solo una letra, usamos comillas simples para que sea un 'char' en C++
    if (text.length === 1) {
...
function generateFullCode() {
    ...
    // Detección de usos
    // NUEVO: Extraemos SOLO las definiciones de funciones, ignorando lo demás
    let userFunctions = '';
    allBlocks.forEach(block => {
        if (block.type === 'procedures_defnoreturn' || block.type === 'procedures_defreturn' || block.type === 'robot_arm_define') {
            userFunctions += Blockly.JavaScript.blockToCode(block);
        }
    });
    ...
    if (varBlocks.length > 0) {
        ...
        varBlocks.forEach(block => {
            ...
            if (!definedVars.has(varName)) {
                ...
                // Si no hay nada conectado, ponemos un valor por defecto seguro según el tipo
                let defaultValue = '0';
                if (varType === 'char') defaultValue = "' '";
                if (varType === 'String') defaultValue = '""';
                if (varType === 'bool') defaultValue = 'false';

                const value = Blockly.JavaScript.valueToCode(block, 'VALUE', Blockly.JavaScript.ORDER_ATOMIC) || defaultValue;
    ...
    code += `// === Customized functions ===\n`;
    code += userFunctions + `\n`; // <-- Aquí pegamos las funciones que extrajimos arriba
